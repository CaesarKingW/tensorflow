...
#ifdef __APPLE__
static const CFStringRef kDriverKextIdentifier = CFSTR("com.nvidia.CL");
#else
...
void Diagnostician::LogDiagnosticInformation() {
  ...
  if (CFDictionaryGetValueIfPresent(kext_infos, kDriverKextIdentifier, (const void**)&cl_driver_info)) {
    bool started = CFBooleanGetValue((CFBooleanRef)CFDictionaryGetValue(cl_driver_info, CFSTR("OSBundleStarted")));
    if (!started) {
  ...
  LOG(INFO) << "retrieving CL diagnostic information for host: "
            << port::Hostname();
...
/* static */ void Diagnostician::LogDriverVersionInformation() {
  ...
  LOG(INFO) << "libcl reported version is: "
...
port::StatusOr<DriverVersion> Diagnostician::FindDsoVersion() {
  port::StatusOr<DriverVersion> result{port::Status{
      ...
      "was unable to find libcl.so DSO loaded into this program"}};

#if defined(__APPLE__)
    // OSX CL libraries have names like: libcl_310.41.15_mercury.dylib
    const string prefix("libcl_");
  ...
  auto iterate_phdr =
    ...
    if (strstr(info->dlpi_name, "libcl.so.1")) {
