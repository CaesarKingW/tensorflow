...
#ifndef TENSORFLOW_STREAM_EXECUTOR_CL_CL_DIAGNOSTICS_H_
#define TENSORFLOW_STREAM_EXECUTOR_CL_CL_DIAGNOSTICS_H_
...
class Diagnostician {
 public:
  // Logs diagnostic information when CL appears to be misconfigured (e.g. is
  // not initializing).
  //
  // Note: if we're running on a machine that has no GPUs, we don't want to
  // produce very much log spew beyond saying, "looks like there's no CL
  // kernel
  // module running".
  //
  ...
  // running on a machine that happens to have a libcl/kernel driver mismatch.
...
#endif  // TENSORFLOW_STREAM_EXECUTOR_CL_CL_DIAGNOSTICS_H_
