...
// The CL-specific DNN library support, implementing the general DnnSupport
// interface.

#ifndef TENSORFLOW_STREAM_EXECUTOR_CL_CL_DNN_H_
#define TENSORFLOW_STREAM_EXECUTOR_CL_CL_DNN_H_
...
extern const PluginId kClDnnPlugin;

// cudnn-library based DNN support. For details on overridden interface
// functions, see dnn.h.
class CldnnSupport : public dnn::DnnSupport {
 public:
  explicit CldnnSupport(CLExecutor* parent);
  ~CldnnSupport() override;
...
//       DeviceMemory<Eigen::half>* backward_bias_data) override;
 ...
 private:
  ...
  CLExecutor* parent_;  // Parent executor object. Not owned.
  ...
  // single cl_dnn translation unit.
  ...
  SE_DISALLOW_COPY_AND_ASSIGN(CldnnSupport);
};
