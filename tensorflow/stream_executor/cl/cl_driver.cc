...
// from the CUcontext to the ClContext* that we pass around internally.
// This also manages assignment of unique ids to ClContexts, to allow
...
class CreatedContexts {
 public:
  ...
  static ClContext* Add(CUcontext context) {
    CHECK(context != nullptr);
    mutex_lock lock{mu_};
    auto cl_context = new ClContext(context, next_id_++);
    Live()->insert(
        make_pair(context, std::unique_ptr<ClContext>(cl_context)));
    return cl_context;
  }
 ...
 private:
  ...
  static std::map<CUcontext, std::unique_ptr<ClContext>> *Live() {
    static auto singleton =
        new std::map<CUcontext, std::unique_ptr<ClContext>>;
    return singleton;
  }
...
CUcontext CurrentContext() {
  CUcontext current  = CLDriver::CurrentContextOrDie();
...
struct ThreadLocalData {
  int64 id;
  ClContext* context;  // Only valid if id == a known good context.
  int depth;
};
...
ScopedActivateContext::ScopedActivateContext(ClContext* cuda_context) {
...
ScopedActivateContext::~ScopedActivateContext() {
  ...
  if (kVerifyClContext) {
    // Note that if kVerifyClContext is used, and contexts are deleted, it's
...
string CUDAPointerToDeviceString(CUdeviceptr pointer) {
  auto value = CLDriver::GetPointerDevice(pointer);
  if (value.ok()) {
...
string CUDAPointerToMemorySpaceString(CUdeviceptr pointer) {
  auto value = CLDriver::GetPointerMemorySpace(pointer);
  if (value.ok()) {
...
string CUDAPointersToCanAccessString(CUdeviceptr from, CUdeviceptr to) {
  auto from_context = CLDriver::GetPointerContext(from);
  if (!from_context.ok()) {
    LOG(ERROR) << "could not retrieve source pointer's context: "
               << from_context.status();
    return "error";
  }
  auto to_context = CLDriver::GetPointerContext(to);
  if (!to_context.ok()) {
    LOG(ERROR) << "could not retrieve destination pointer's context: "
               << to_context.status();
    return "error";
  }
  return CLDriver::CanEnablePeerAccess(from_context.ValueOrDie(),
...
/* static */ port::Status CLDriver::Init() {
  // Cached return value from calling InternalInit(), as cuInit need only be
  // called once, but CLDriver::Init may be called many times.
...
/* static */ bool CLDriver::GetDeviceName(CUdevice device,
                                            string *device_name) {
...
/* static */ port::Status CLDriver::CreateContext(
    CUdevice device, DeviceOptions device_options, ClContext** context) {
  *context = nullptr;
...
/* static */ void CLDriver::DestroyContext(ClContext* context) {
  if (context == nullptr) {
    return;
  }

  CUresult res = CUDA_ERROR_UNKNOWN;
  // CUresult res = cuCtxDestroy_v2(context->context());
  if (res != CUDA_SUCCESS) {
...
/* static */ bool CLDriver::FuncGetAttribute(CUfunction_attribute attribute,
                                               CUfunction func,
                                               int *attribute_value) {
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ bool CLDriver::FuncSetCacheConfig(CUfunction function,
...
CLDriver::ContextGetSharedMemConfig(ClContext* context) {
...
/* static */ port::Status CLDriver::ContextSetSharedMemConfig(
...
/* static */ bool CLDriver::LaunchKernel(
    ClContext* context, CUfunction function, unsigned int grid_dim_x,
...
/* static */ port::Status CLDriver::LoadCubin(ClContext* context,
...
/* static */ bool CLDriver::LoadPtx(ClContext* context,
...
/* static */ bool CLDriver::SynchronousMemsetUint8(ClContext* context,
...
/* static */ bool CLDriver::SynchronousMemsetUint32(ClContext* context,
                                                      CUdeviceptr location,
                                                      uint32 value,
                                                      size_t uint32_count) {
  ScopedActivateContext activation{context};
...
/* static */ bool CLDriver::AsynchronousMemsetUint8(ClContext* context,
                                                      CUdeviceptr location,
                                                      uint8 value,
                                                      size_t uint32_count,
                                                      CUstream stream) {
...
/* static */ bool CLDriver::AsynchronousMemsetUint32(ClContext* context,
                                                       CUdeviceptr location,
                                                       uint32 value,
                                                       size_t uint32_count,
                                                       CUstream stream) {
...
/* static */ bool CLDriver::AddStreamCallback(ClContext* context,
                                                CUstream stream,
...
/* static */ bool CLDriver::GetModuleFunction(ClContext *context,
                                                CUmodule module,
...
/* static */ bool CLDriver::GetModuleSymbol(ClContext* context,
                                              CUmodule module,
  ...
  CUresult res = CUDA_ERROR_UNKNOWN;
  // CUresult res =
  //     cuModuleGetGlobal_v2(dptr, bytes, module, symbol_name);
  if (res != CUDA_SUCCESS) {
...
/* static */ void CLDriver::UnloadModule(ClContext *context,
                                           CUmodule module) {
  ScopedActivateContext activated{context};
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ port::StatusOr<CUdevice> CLDriver::DeviceFromContext(
    ClContext* context) {
  ScopedActivateContext activated{context};
  CUdevice device = -1;
  CUresult result = CUDA_ERROR_UNKNOWN;
...
/* static */ bool CLDriver::CreateStream(ClContext *context,
                                           CUstream *out) {
...
/* static */ void CLDriver::DestroyStream(ClContext* context,
                                            CUstream *stream) {
  if (*stream == nullptr) {
    return;
  }
...
/* static */ void CLDriver::DeviceDeallocate(ClContext* context,
                                               void *location) {
  std::cout << "cl_driver DeviceDeallocate " << location << std::endl;
  ScopedActivateContext activation{context};
...
/* static */ void *CLDriver::HostAllocate(ClContext *context,
                                            uint64 bytes) {
  ScopedActivateContext activation{context};
...
/* static */ void CLDriver::HostDeallocate(ClContext* context,
                                             void *location) {
  ScopedActivateContext activation{context};
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ bool CLDriver::HostRegister(ClContext* context, void *location,
                                           uint64 bytes) {
  ScopedActivateContext activation{context};
  // "Portable" memory is visible to all CUDA contexts. Safe for our use model.
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ bool CLDriver::HostUnregister(ClContext* context,
                                             void *location) {
  ScopedActivateContext activation{context};
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ port::Status CLDriver::DestroyEvent(ClContext* context,
                                                   CUevent *event) {
  if (*event == nullptr) {
...
/* static */ port::Status CLDriver::RecordEvent(ClContext* context,
                                                  CUevent event,
                                                  CUstream stream) {
  ScopedActivateContext activated{context};
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ port::StatusOr<CUresult> CLDriver::QueryEvent(
...
/* static */ bool CLDriver::GetEventElapsedTime(ClContext* context,
...
/* static */ bool CLDriver::WaitStreamOnEvent(ClContext* context,
                                                CUstream stream,
                                                CUevent event) {
  ScopedActivateContext activation{context};
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ bool CLDriver::AsynchronousMemcpyH2D(ClContext* context,
                                                    CUdeviceptr gpu_dst,
                                                    const void *host_src,
                                                    uint64 size,
                                                    CUstream stream) {
...
/* static */ bool CLDriver::AsynchronousMemcpyD2D(ClContext* context,
                                                    CUdeviceptr gpu_dst,
                                                    CUdeviceptr gpu_src,
                                                    uint64 size,
                                                    CUstream stream) {
...
/* static */ port::Status CLDriver::CreateEvent(ClContext* context,
                                                  CUevent *result,
                                                  EventFlags flags) {
  int cuflags;
  switch (flags) {
...
/* static */ port::StatusOr<ClContext*> CLDriver::GetPointerContext(
    CUdeviceptr pointer) {
...
/* static */ port::StatusOr<MemorySpace> CLDriver::GetPointerMemorySpace(
    CUdeviceptr pointer) {
  unsigned int value;
  CUresult result = CUDA_ERROR_UNKNOWN;
  // CUresult result = cuPointerGetAttribute(
...
/* static */ port::Status CLDriver::GetPointerAddressRange(CUdeviceptr dptr,
                                                             CUdeviceptr *base,
                                                             size_t *size) {
  CUresult result = CUDA_ERROR_UNKNOWN;
...
/* static */ port::StatusOr<CUdevice> CLDriver::GetPointerDevice(
    CUdeviceptr pointer) {
...
/* static */ port::Status CLDriver::GetComputeCapability(int *cc_major,
                                                           int *cc_minor,
                                                           CUdevice device) {
  *cc_major = 0;
  *cc_minor = 0;
...
/* static */ port::StatusOr<int> CLDriver::GetMultiprocessorCount(
    CUdevice device) {
  return GetSimpleAttribute<int>(device,
                                 CU_DEVICE_ATTRIBUTE_MULTIPROCESSOR_COUNT);
}

/* static */ port::StatusOr<int64> CLDriver::GetMaxSharedMemoryPerCore(
    CUdevice device) {
...
/* static */ port::StatusOr<int64> CLDriver::GetMaxThreadsPerBlock(
    CUdevice device) {
  return GetSimpleAttribute<int64>(device,
                                   CU_DEVICE_ATTRIBUTE_MAX_THREADS_PER_BLOCK);
}

/* static */ port::StatusOr<int64> CLDriver::GetMaxRegistersPerBlock(
    CUdevice device) {
  return GetSimpleAttribute<int64>(device,
                                   CU_DEVICE_ATTRIBUTE_MAX_REGISTERS_PER_BLOCK);
}

/* static */ port::StatusOr<int64> CLDriver::GetThreadsPerWarp(
    CUdevice device) {
  return GetSimpleAttribute<int64>(device, CU_DEVICE_ATTRIBUTE_WARP_SIZE);
}

/* static */ bool CLDriver::GetGridLimits(int *x, int *y, int *z,
                                            CUdevice device) {
  int value;
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ bool CLDriver::GetDeviceMemoryInfo(ClContext* context,
                                                  int64 *free_out,
                                                  int64 *total_out) {
  ScopedActivateContext activation{context};
  size_t free = 0;
...
/* static */ bool CLDriver::GetDeviceTotalMemory(CUdevice device,
                                                   uint64 *result) {
  size_t value = -1;
  CUresult res = CUDA_ERROR_UNKNOWN;
...
/* static */ string CLDriver::GetPCIBusID(CUdevice device) {
  string pci_bus_id;
...
/* static */ bool CLDriver::CanEnablePeerAccess(ClContext* from,
                                                  ClContext* to) {
  if (from == to) {
...
/* static */ port::Status CLDriver::EnablePeerAccess(ClContext* from,
                                                       ClContext* to) {
  if (from == to) {
  ...
  CUresult result = CUDA_ERROR_UNKNOWN;
  // CUresult result =
  //     cuCtxEnablePeerAccess(to->context(), 0 /* = flags */);
  if (result != CUDA_SUCCESS &&
...
/* static */ port::StatusOr<int> CLDriver::GetMaxOccupiedBlocksPerCore(
