...
// CL userspace driver library wrapper functionality.

#ifndef TENSORFLOW_STREAM_EXECUTOR_CL_CL_DRIVER_H_
...
// CLDriver::GetPointerMemorySpace().
...
// CLDriver contains wrappers for calls to the userspace library driver. It's
...
//
// The order of parameters is generally kept symmetric with the underlying CL
// driver API.
//
// Links on functions are to specific documentation under
// http://docs.nvidia.com/cl/cl-driver-api/
//
...
class CLDriver {
 public:
  ...
  // after the first.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__INITIALIZE.html#group__CL__INITIALIZE_1g0a2f1517e1bd8502c7194c3a8c134bc3
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1g4e84b109eba36cdaaade167f34ae881e
  ...
  // Destroys a CL stream associated with the given context.
  // stream is owned by the caller, must not be null, and *stream is set to null
  // if the stream is successfuly destroyed.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__STREAM.html#group__CL__STREAM_1g244c8833de4596bcd31a06cdf21ee758
  static void DestroyStream(ClContext* context, CUstream *stream);

  // CL events can explicitly disable event TSC retrieval for some presumed
  // performance improvement if timing is unnecessary.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EVENT.html#group__CL__EVENT_1g450687e75f3ff992fe01662a43d9d3db
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EVENT.html#group__CL__EVENT_1g450687e75f3ff992fe01662a43d9d3db
  static port::Status CreateEvent(ClContext* context, CUevent *result,
                                  EventFlags flags);

  // Destroys *event and turns it into a nullptr. event may not be null, but
  // *event may be, via cuEventDestroy
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EVENT.html#group__CL__EVENT_1g593ec73a8ec5a5fc031311d3e4dca1ef
  static port::Status DestroyEvent(ClContext* context, CUevent *event);

  // Allocates a GPU memory space of size bytes associated with the given
  // context via cuMemAlloc.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1gb82d2a09844a58dd9e744dc31e8aa467
  static void *DeviceAllocate(ClContext* context, uint64 bytes);

  // Deallocates a GPU memory space of size bytes associated with the given
  // context via cuMemFree.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g89b3f154e17cc89b6eea277dbdf5c93a
  static void DeviceDeallocate(ClContext* context, void *location);

  // Allocates page-locked and CL-registered memory on the host via
  // cuMemAllocHost.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1gdd8311286d2c2691605362c689bc64e0
  static void *HostAllocate(ClContext* context, uint64 bytes);

  // Deallocates a location created by HostAllocate, via cuMemFreeHost.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g62e0fdbe181dab6b1c90fa1a51c7b92c
  static void HostDeallocate(ClContext* context, void *location);

  // Registers a memory region at location of size bytes via cuMemHostRegister.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1gf0a9fe11544326dabd743b7aa6b54223
  static bool HostRegister(ClContext* context, void *location, uint64 bytes);

  // Unregisters a memory region that was previously registered at location via
  // cuMemHostUnregister.
  //
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g63f450c8125359be87b7623b1c0b2a14
  //
  // TODO(leary) verify an error will be returned if the location wasn't
  // previously registered.
  static bool HostUnregister(ClContext* context, void *location);
  ...
  //
  // N.B. CL contexts are weird. They are implicitly associated with the
  // calling thread. Current documentation on contexts and their influence on
  // userspace processes is given here:
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1g65dc0012348bc84810e2103a40d8e2cf
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1g27a365aebb0eb548166309f58a1e8b8e
  static void DestroyContext(ClContext* context);

  // Queries the runtime for the specified attribute of the specified function.
  // cuFuncGetAttribute (the underlying CL driver API routine) only operates
  // in terms of integer-sized values, so there's no potential for overrun (as
  // of CL 5.5).
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EXEC.html#group__CL__EXEC_1g5e92a1b0d8d1b82cb00dcfb2de15961b
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EXEC.html#group__CL__EXEC_1g40f8c11e81def95dc0072a375f965681
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1g17153a1b8b8c756f7ab8505686a4ad74
  ...
  // CONTEXT (not function!), either default or four- or eight-byte bank size.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1g2574235fa643f8f251bf7bc28fac3692
  static port::Status ContextSetSharedMemConfig(
      ClContext* context, CUsharedconfig shared_mem_config);

  // Launches a CL kernel via cuLaunchKernel.
  // TODO(leary) describe the structure of kernel_params and extra in a readable
  // way.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EXEC.html#group__CL__EXEC_1gb8f3dc3031b40da29d5f9a7139e52e15
  static bool LaunchKernel(ClContext* context, CUfunction function,
  ...
  // Loads ptx_contents with the CL driver's PTX JIT and stores the resulting
  // handle in "module". Any error logs that are produced are logged internally.
  static bool LoadPtx(ClContext* context, const char *ptx_contents,
                      CUmodule *module);

  // Loads cubin_bytes with the CL driver's blob loading interface and stores
  // the resulting handle in "module".
  static port::Status LoadCubin(ClContext* context, const char *cubin_bytes,
                                CUmodule *module);

  // Retrieves a named kernel from a loaded module, and places the resulting
  // handle into function (outparam) on success. Neither kernel_name nor
  // function may be null. No ownership is taken of kernel_name.
  static bool GetModuleFunction(ClContext* context, CUmodule module,
  ...
  static bool GetModuleSymbol(ClContext* context, CUmodule module,
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MODULE.html#group__CL__MODULE_1g8ea3d716524369de3763104ced4ea57b
  static void UnloadModule(ClContext* context, CUmodule module);

  // Performs a synchronous memset of the device memory segment via cuMemsetD8.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g6e582bf866e9e2fb014297bfaf354d7b
  static bool SynchronousMemsetUint8(ClContext* context, CUdeviceptr location,
                                     uint8 value, size_t size);

  // Performs a synchronous memset of the device memory segment via cuMemsetD32.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g983e8d8759acd1b64326317481fbf132
  static bool SynchronousMemsetUint32(ClContext* context,
                                      CUdeviceptr location, uint32 value,
                                      size_t uint32_count);

  // Performs an asynchronous memset of the device memory segment via
  // cuMemsetD8Async.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1gaef08a7ccd61112f94e82f2b30d43627
  static bool AsynchronousMemsetUint8(ClContext* context, CUdeviceptr location,
                                      uint8 value, size_t uint32_count,
                                      CUstream stream);

  // Performs an asynchronous memset of the device memory segment via
  // cuMemsetD32Async.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g58229da5d30f1c0cdf667b320ec2c0f5
  static bool AsynchronousMemsetUint32(ClContext* context,
                                       CUdeviceptr location, uint32 value,
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g56f30236c7c5247f8e061b59d3268362

  static bool AsynchronousMemcpyD2H(ClContext* context, void *host_dst,
                                    CUdeviceptr gpu_src, uint64 size,
                                    CUstream stream);
  static bool AsynchronousMemcpyH2D(ClContext* context, CUdeviceptr gpu_dst,
                                    const void *host_src, uint64 size,
                                    CUstream stream);
  static bool AsynchronousMemcpyD2D(ClContext* context, CUdeviceptr gpu_dst,
                                    CUdeviceptr gpu_src, uint64 size,
                                    CUstream stream);

  // The CL stream callback type signature.
  // The data passed to AddStreamCallback is subsequently passed to this
  // callback when it fires.
  //
  // Some notable things:
  // * Callbacks must not make any CL API calls.
  // * Callbacks from independent streams execute in an undefined order and may
  //   be serialized.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__STREAM.html#group__CL__STREAM_1g613d97a277d7640f4cb1c03bd51c2483
  ...
  // details.
  static bool AddStreamCallback(ClContext* context, CUstream stream,
  ...
  //
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1g7a54725f28d34b8c6299f0c6ca579616
  static bool SynchronizeContext(ClContext* context);
  ...
  static bool IsStreamIdle(ClContext* context, CUstream stream);

  // Returns whether code in the from context can access memory in the to
  // context via cuDeviceCanAccessPeer.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__PEER__ACCESS.html#group__CL__PEER__ACCESS_1g496bdaae1f632ebfb695b99d2c40f19e
  static bool CanEnablePeerAccess(ClContext* from, ClContext* to);

  // Enables peer access per CanEnablePeerAccess, via cuCtxEnablePeerAccess.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__PEER__ACCESS.html#group__CL__PEER__ACCESS_1g0889ec6728e61c05ed359551d67b3f5a
  static port::Status EnablePeerAccess(ClContext* from, ClContext* to);

  // Returns the elapsed milliseconds between start and stop via
  // cuEventElapsedTime.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EVENT.html#group__CL__EVENT_1gdfb1178807353bbcaa9e245da497cf97
  static bool GetEventElapsedTime(ClContext* context,
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EVENT.html#group__CL__EVENT_1g95424d3be52c4eb95d83861b70fb89d1
  static port::Status RecordEvent(ClContext* context, CUevent event,
                                  CUstream stream);

  // Polls (without blocking) to determine the status of an event - pending or
  // complete (or an error status).
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__EVENT.html#group__CL__EVENT_1g6f0704d755066b0ee705749ae911deef
  static port::StatusOr<CUresult> QueryEvent(ClContext* context,
                                             CUevent event);

  // -- Pointer-specific calls.

  // Returns the context in which pointer was allocated or registered.
  static port::StatusOr<ClContext*> GetPointerContext(CUdeviceptr pointer);
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__DEVICE__DEPRECATED.html#group__CL__DEVICE__DEPRECATED_1ge2091bbac7e1fb18c2821612115607ea
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__DEVICE.html#group__CL__DEVICE_1g9c3e1414f0ad901d3278a4d6645fc266
  ...
  //
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__DEVICE__DEPRECATED.html#group__CL__DEVICE__DEPRECATED_1g65a5b4e25186bd257df80b98c98cffe6
  ...
  // Returns the total amount of memory available for allocation by the CL
  ...
  // by cuMemGetInfo.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g808f555540d0143a331cc42aa98835c0
  static bool GetDeviceMemoryInfo(ClContext* context, int64* free,
                                  int64* total);

  // Returns a PCI bus id string for the device.
  // [domain]:[bus]:[device].[function]
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__MEM.html#group__CL__MEM_1g85295e7d9745ab8f0aa80dd1e172acfc
  static string GetPCIBusID(CUdevice device);

  // -- Context- and device-independent calls.

  // Returns the number of visible CL device via cuDeviceGetCount.
  // This should correspond to the set of device ordinals available.
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__DEVICE.html#group__CL__DEVICE_1g52b5ce05cb8c5fb6831b2c0ff2887c74
  ...
  // instead, the CL toolkit release number that this driver is compatible
  // with; e.g. 6000 (for a CL 6.0 compatible driver) or 6050 (for a CL 6.5
  // compatible driver).
  //
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__VERSION.html#group__CL__VERSION_1g8b7a10395392e049006e61bcdc8ebe71
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__OCCUPANCY.html#group__CL__OCCUPANCY_1gcc6e1094d05cba2cee17fe33ddd04a98
  static port::StatusOr<int> GetMaxOccupiedBlocksPerCore(
      ClContext* context, CUfunction kernel, int threads_per_block,
      size_t dynamic_shared_memory_bytes);

  // Returns the current context set in CL. This is done by calling the cl
  // driver (e.g., this value is not our cached view of the current context).
  static CUcontext CurrentContextOrDie();

  // Seam for injecting an error at CL initialization time for testing
  // purposes.
...
class ScopedActivateContext {
 public:
  ...
  // http://docs.nvidia.com/cl/cl-driver-api/group__CL__CTX.html#group__CL__CTX_1gbe562ee6258b4fcc272ca6478ca2a2f7
  explicit ScopedActivateContext(ClContext* context);
 ...
 private:
  ClContext* to_restore_ = nullptr;
};

// ClContext wraps a cl CUcontext handle, and includes a unique id. The
// unique id is positive, and ids are not repeated within the process.
class ClContext {
 public:
  ClContext(CUcontext context, int64 id) : context_(context), id_(id) { }
...
#endif  // TENSORFLOW_STREAM_EXECUTOR_CL_CL_DRIVER_H_
